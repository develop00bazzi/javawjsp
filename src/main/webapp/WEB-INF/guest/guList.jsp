...
<div class="container">
	...
	<table class="table table-borderless m-0 p-0">
		<tr>
			...
			<td style="text-align:right;">
				...
				<c:if test="${pag>1}">
					<a href="${ctp}/guList.gu?pag=1&pageSize=${pageSize}" class="btn-sm btn-secondary">첫 페이지</a>
					<a href="${ctp}/guList.gu?pag=${pag-1}&pageSize=${pageSize}" class="btn-sm btn-secondary">이전 페이지</a>
				</c:if>
				${pag}/${totPage}
				<c:if test="${pag<totPage}">
					<a href="${ctp}/guList.gu?pag=${pag+1}&pageSize=${pageSize}"  class="btn-sm btn-secondary"> 다음 페이지</a>
					<a href="${ctp}/guList.gu?pag=${totPage}&pageSize=${pageSize}"  class="btn-sm btn-secondary"> 마지막 페이지</a>
				</c:if>
			</td>
		</tr>
	</table>
	...
	<div class="text-center">
		<ul class="pagination justify-content-center">
			<c:if test="${pag>1}">
				<li class="page-item"><a class="page-link text-secondary" href="${ctp}/guList.gu?pag=1&pageSize=${pageSize}" class="btn-sm btn-secondary">첫 페이지</a></li>
			</c:if>
			<c:if test="${curBlock>0}">
				<li class="page-item"><a class="page-link text-secondary" href="${ctp}/guList.gu?pag=${(curBlock-1)*blockSize+1}&pageSize=${pageSize}" class="btn-sm btn-secondary">이전 블록</a></li>
			</c:if>
			<c:forEach var="i" begin="${(curBlock)*blockSize+1}" end="${(curBlock)*blockSize+blockSize}" varStatus="st">
				<c:if test="${i<=totPage && i==pag}">
					<li class="page-item active"><a class="page-link bg-secondary border-secondary" href="${ctp}/guList.gu?pag=${i}&pageSize=${pageSize}" class="btn-sm btn-secondary">${i}</a></li>
				</c:if>
				<c:if test="${i<=totPage && i!=pag}">
					<li class="page-item"><a class="page-link text-secondary" href="${ctp}/guList.gu?pag=${i}&pageSize=${pageSize}" class="btn-sm btn-secondary">${i}</a></li>
				</c:if>
			</c:forEach>
			<c:if test="${curBlock<lastBlock}">
				<li class="page-item"><a class="page-link text-secondary" href="${ctp}/guList.gu?pag=${(curBlock+1)*blockSize+1}&pageSize=${pageSize}" class="btn-sm btn-secondary">다음 블록</a></li>
			</c:if>
			<c:if test="${pag<totPage}">
				<li class="page-item"><a class="page-link text-secondary" href="${ctp}/guList.gu?pag=${totPage}&pageSize=${pageSize}" class="btn-sm btn-secondary">마지막 페이지</a></li>
			</c:if>
		</ul>
	</div>
</div>
