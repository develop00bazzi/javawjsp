...
<div class="container">
	...
	<div class="text-center">
		...
		<c:if test="${curBlock>0}">
			<a href="${ctp}/guList.gu?pag=${(curBlock-1)*blockSize+1}" class="btn-sm btn-secondary">이전 블록</a>
		</c:if>
		<c:forEach var="i" begin="${(curBlock)*blockSize+1}" end="${(curBlock)*blockSize+blockSize}" varStatus="st">
			<c:if test="${i<=totPage}">
				<a href="${ctp}/guList.gu?pag=${i}" class="btn-sm btn-secondary">${i}</a>
			</c:if>
		</c:forEach>
		<c:if test="${curBlock<lastBlock}">
			<a href="${ctp}/guList.gu?pag=${(curBlock+1)*blockSize+1}" class="btn-sm btn-secondary">다음 블록</a>
		</c:if>
		<c:if test="${pag<totPage}">
			<a href="${ctp}/guList.gu?pag=${totPage}" class="btn-sm btn-secondary">마지막 페이지</a>
		</c:if>
