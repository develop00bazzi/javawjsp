...
<div class="container">
	...
	<p>
		...
		4-1. 문자열 추출 1 (substring()): <%=atom.substring(0,3)%> / ${fn:substring(atom,0,3)} <br/>
		4-2. 문자열 추출 2 (substring()): <%=atom.substring(2)%> / <%-- ${fn:substring(atom,2)} --%> 
																	 ${fn:substring(atom,2, fn:length(atom))}<br/>
		<%-- 4-3. 문자열 추출 3 (substring()): <%=atom.substring(2)%> / ${fn:substring(atom,2,-1}<br/> --%>
		5. 특정 문자열의 위치값 찾기(indexOf()): <%=atom.indexOf("o")%> / ${fn:indexOf(atom,'o')} <br/>
		5-2. atom 변수에 기억되어 있는 'o' 문자의 위치를 모두 출력하시오. <br/>
		<br/>
		...
		<p>문자 'o'의 전체 개수는? ${cnt}입니다. <p><br/>
		<br/>
		5-3. atom 변수에 기억되어 있는 문자를 입력받아 그 문자가 두번째로 기억되어 있는 위치 값을 출력하시오.<br/>
		...
		6-2. atom 변수에 기억되어 있는 문자를 입력받아 그 문자가 두번째로 기억되어 있는 위치 값을 출력하시오.<br/>
			 단, 두번째 기억 값이 없으면 1번째 기억 위치를 출력하고 '두번째 값 없음' 을 함께 출력하시오.
		...
		7. 문자열 분리 함수(split(변수, 분리할 문자) => 결과를 변수에 담아주어야 한다.) <br/>
		 atom 변수의 문자 중 'o' 문자를 기준으로 분리 처리 <br/>
		 <c:set var="atoms" value="${fn:split(atom,'o')}"/>
		 <c:forEach var="atom" items="${atoms}" varStatus="st">
		 	${st.count}. ${atom}<br/>
		 </c:forEach>
		 <br/>
		 atoms의 전체 길이는? ${fn:length(atoms)} <br/>
		...
		8. 문자열 치환(replace())<br/>
		 문제: atom2 변수 안의 'My'를 'Your'로 변경하시오. :
		 <%=atom.replace("My", "Your") %> / ${fn:replace(atom2,'My','Your')} <br/>
		
		9. 특정 문자 포함 유무 확인 contaions()<br/>
		 문제 1: atom2 변수에 'o'를 포함하고 있느냐? ${fn:contains(atom2,'o')}<br/>
		 문제 2: atom2 변수에 'My'를 포함하고 있느냐? ${fn:contains(atom2,'My')}<br/>
		 문제 3: atom2 변수에 'Your'를 포함하고 있느냐? ${fn:contains(atom2,'Your')}<br/>
